...
//
//assign each time with a characteristics, start and end, and sort all start and end time, traverse all time, if the time
//        is a start time, busy_employee puls 1, otherwise minus 1, when busy employee is 0, there is a new interval
//        time: O(MlogM)
...
public class meetingTime {
    ...
    static class getAvailableIntervals{
        ...
        public List<Interval> getAvailableFollow(List<List<Interval>> intervals, int k) {
            List<Point> points = new ArrayList<>();
            for(List<Interval> eachPerson: intervals) {
                for(Interval eachInterval:eachPerson) {
                    points.add(new Point(eachInterval.start, true));
    ...
    public static void main(String[] args) {
        ...
        getAvailableIntervals getInterval = new getAvailableIntervals();
        List<Interval> res = getInterval.getAvailableIntervals(intervals);
        for(Interval each : res) {
            System.out.println("[" + each.start + "," + each.end + "]");
        }

        System.out.println(" ");
        List<Interval> res2 = getInterval.getAvailableFollow(intervals, 2);
